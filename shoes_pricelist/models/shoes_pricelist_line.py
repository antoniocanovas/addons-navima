...
class ShoesPricelist(models.Model):
    ...
    product_tmpl_single_id = fields.Many2one('product.template', string='Model', domain="[('is_pair','=',True)]")
    pricelist_price = fields.Float('Price')

    image = fields.Binary(related='product_tmpl_single_id.image_1024')
    product_single_price = fields.Float(related='product_tmpl_single_id.list_price', string='List price')
    product_standard_price = fields.Monetary(related='product_tmpl_single_id.exwork_euro', string='Exwork')
    ...
    currency_id = fields.Many2one('res.currency', store=True, default=lambda self: self.env.user.company_id.currency_id)
