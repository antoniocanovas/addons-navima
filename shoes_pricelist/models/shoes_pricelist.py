...
class ShoesPricelist(models.Model):
    ...
    shoes_campaign_id = fields.Many2one('project.project', string='Campaign', required=True)
    pricelist_id = fields.Many2one('product.pricelist', string='Pricelist', required=True)
    date = fields.Date('Date', default=lambda self: fields.Date.context_today(self), required=True)
    active = fields.Boolean('Active', default=True)
    notes = fields.Html('Notes')
    line_ids = fields.One2many('shoes.pricelist.line', 'shoes_pricelist_id', string='Lines')
    ...
    def update_shoes_pricelist(self):
        ...
        for model in models:
            ...
            self.env['shoes.pricelist.line'].create({'shoes_pricelist_id':self.id,
